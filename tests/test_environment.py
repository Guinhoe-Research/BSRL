...
def make_action(idx: int) -> torch.Tensor:
    """Create a one-hot action tensor with the given index set."""
...
def play_claim_and_select(env, claim_idx, *select_idxs):
    ...
    env.step(make_action(claim_idx))
    obs = None
    for idx in select_idxs:
        obs, *_ = env.step(make_action(idx))
    return obs
...
def test_claim_phase(env):
    seed_and_reset(env)
    obs, *_ = env.step(make_action(CLAIM_2))
...
def test_pass_advances_round(env):
    ...
    # agent_1 passes
    obs, _, terminated, *_ = env.step(make_action(PASS))
...
def test_challenge_untruthful_claim(env):
    ...
    obs, reward, terminated, *_ = env.step(make_action(CHALLENGE))
...
def test_challenge_truthful_claim(env):
    ...
    play_claim_and_select(env, CLAIM_1, SELECT_RANK_3)
    env.step(make_action(CHALLENGE))
...
def test_invalid_action_in_claim_phase(env):
    ...
    with pytest.raises(ValueError):
        env.step(make_action(CHALLENGE))  # can't challenge during CLAIM


def test_select_card_not_in_hand(env):
    seed_and_reset(env)
    env.step(make_action(CLAIM_1))
    # rank 9 (idx 8, action 12) — agent_0 has 0 cards of rank 9
    with pytest.raises(ValueError):
        env.step(make_action(12))
