...
MAX_AGENTS = 5
MIN_AGENTS = 2
env_cfg = EnvironmentConfig(num_agents=MIN_AGENTS, SEE_CARD_COUNTS=True, max_agents=MAX_AGENTS)
...
reward_cfg = RewardConfig(
    ...
    truthful_card_bonus=1.2,   # incentivize playing correct rank
    successful_bluff=1.5,       # reward getting away with BS
    challenge_cost=0.2,        # tax on challenge spam
    win_bonus=10.0,
)
...
ACTION_DIM = 19
OBSERVATION_DIM = 20 + MAX_AGENTS if env_cfg.SEE_CARD_COUNTS else 20

# Curriculum: map each epoch to a num_agents value (2 → 5)
EPOCHS_PER_STAGE = train_cfg.num_epochs // (MAX_AGENTS - MIN_AGENTS + 1)

def curriculum_num_agents(epoch: int) -> int:
    stage = min(epoch // EPOCHS_PER_STAGE, MAX_AGENTS - MIN_AGENTS)
    return MIN_AGENTS + stage
...
def obtain_trajectories(env, epoch=0):
    observations = []
    actions      = []
    log_probs    = []
    values       = []
    ...
    obs, _ = env.reset()
    modified_traj_window = train_cfg.trajectory_window + 64 * (curriculum_num_agents(epoch) - MIN_AGENTS)  # give extra steps for later stages with more agents
    for _ in range(modified_traj_window):
    ...
    advantages = torch.zeros(modified_traj_window, dtype=torch.float32)
    last_gae = 0.0
    ...
    for t in reversed(range(modified_traj_window)):
        next_value          = last_value if t == modified_traj_window - 1 else values[t + 1]
...
def vec_collection(num_envs, cur_env_cfg):
    v_observations = []
    v_actions      = []
    v_log_probs    = []
    v_values       = []
    ...
    for _ in range(num_envs):
        new_env = Environment(cur_env_cfg, reward_cfg)
        o, a, lp, v, adv, r, am, cs = obtain_trajectories(new_env, epoch=EPOCH)
...
for epoch in range(train_cfg.num_epochs):
    # Curriculum: update num_agents for this epoch
    EPOCH = epoch
    cur_num_agents = curriculum_num_agents(epoch)
    cur_env_cfg = EnvironmentConfig(
        num_agents=cur_num_agents,
        SEE_CARD_COUNTS=env_cfg.SEE_CARD_COUNTS,
        ENCODE_CLAIMANTS=env_cfg.ENCODE_CLAIMANTS,
        POSITIONAL_EMBEDDINGS=env_cfg.POSITIONAL_EMBEDDINGS,
        max_agents=MAX_AGENTS,
    )
    print(f"Epoch {epoch + 1}/{train_cfg.num_epochs}  (num_agents={cur_num_agents})")
    ### Trajectory Farming
    # Collect trajectories
    observations, actions, log_probs, values, advantages, returns, action_masks, claim_sequences = vec_collection(train_cfg.num_envs, cur_env_cfg)
...
torch.save(training_metadata, output_dir / "ppo_training_metadata.pth")
torch.save(ppomodel.state_dict(), output_dir / "ppo_model_100_50.pth")

# Also save configs as human-readable JSON for easy editing
save_configs(
    output_dir / "training_configs.json",
    env=env_cfg,
    reward=reward_cfg,
    training=train_cfg,
)
